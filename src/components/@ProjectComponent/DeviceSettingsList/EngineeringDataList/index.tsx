...
const DeviceSettingList = ({
  ...
  const initlizeApp = async () => {
    consoleLog('settingsData -->', settingsData);
    let __note = settingsData?.engineeringData?.value ?? '';
    consoleLog("settingsData==>", __note.toString("utf8").length);
    consoleLog('__note resultObj==>', deviceSettingsData);

    // Handle unsaved value which were changed
    const resultObj = findObject('engineeringData', deviceSettingsData?.EngineeringData, {
      searchKey: 'name',
    });
